...
def copiar_archivos_desde_ftp(archivos, carpeta_ftp, carpeta_local):
    ...
    ftp.quit()
    if carpeta_local == r'C:\Analisis_Masivo_guia\Proceso_Clidad_1\chat':
        if archivos_no_encontrados:
            ...
            ruta_no_encontrados = os.path.join(r'C:\Analisis_Masivo_guia\Proceso_Clidad_1\registro no encontrados', f'audios_no_descargados_{fecha_hoy}.txt')
...
cursor = conexion.cursor()
consulta = ("SELECT audio_name FROM audios WHERE tipo = 'soporte' and status in ('Transcrito','Reprocesar') and guia = 'guia_set_11' ORDER BY id ASC LIMIT 1")
